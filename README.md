...
1. **Limpeza Inicial:** Para garantir um início limpo, remova quaisquer arquivos de token antigos que possam ter sido criados:
...
2. **Obter um NOVO e VIRGEM `authorization_code`:**
...
3. **Atualize seu `.env` com o `GHL_AUTHORIZATION_CODE`:**
...
4. **Execute o script Python localmente:**
    ...
    **Saída Esperada:** Você verá mensagens indicando a busca de tokens, a troca do código de autorização, e finalmente:
...
**Saída Esperada:** Similar à execução local, mostrando o carregamento de tokens, a adição da tag e o sucesso do health check.
...
3. **Copiar `.env` e `ghl_tokens.json` para a VPS:**
...
2. **Tornar `run_ghl_check.sh` executável:**
    
    ```
    chmod +x run_ghl_check.sh
    ```
    
3. **Criar `setup_cron.sh`:**
...
4. **Tornar `setup_cron.sh` executável:**
...
2. **Teste a Execução Manual (Opcional, mas recomendado):** Execute o script uma vez para confirmar que tudo está funcionando:
...
Agora, para **commitar essa última alteração** (o `README.md` completo) para o seu repositório GitHub:

1. **No seu terminal LOCAL (no seu computador, não na VPS):**
    
2. **Certifique-se de estar na raiz do seu projeto:**
    
    ```
    cd C:\Users\WEB\OneDrive\Área de Trabalho\ghl-api-auth-template
    ```
    
3. **Adicione o `README.md` alterado ao "stage" do Git:**
    
    ```
    git add README.md
    ```
    
    (Se você fez outras pequenas alterações em outros arquivos que deseja incluir no commit, pode usar `git add .` para adicionar todos os arquivos modificados.)
    
4. **Crie o commit:**
    
    ```
    git commit -m "docs: Adiciona README.md completo explicando todo o projeto e deploy."
    ```
...
Parabéns! Com isso, seu repositório estará totalmente atualizado com a documentação completa do seu projeto de autenticação GoHighLevel.
